...
void dashboard(unsigned char key) {
        ...
        clcd_print(event[pos],(0xC0 + 10));

        adc_val=read_adc(0)/10.23;
...
void login_screen(unsigned char *oper)
{
    ...
    if(lock_flag)
    {
        ...
        clcd_print("  User Blocked", (0x80 + 0));
        clcd_print("  Wait 30 Sec's", (0xC0 + 0));
    ...
    if (login_index < 4)
    {
        ...
        if (login_delay < 5)
            clcd_putch('_', (0xC0 + login_index+4));
        else if (login_delay < 10)
            clcd_putch(' ', (0xC0 + login_index+4));
        else
            login_delay = 0;
